  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      <div className="flex justify-between items-center pb-4 border-b border-gray-100">
          <h3 className="text-xl font-bold text-gray-800">{initialData ? 'Edit Recipe' : 'New Recipe'}</h3>
          {onCancel && (
              <button 
                type="button" 
                onClick={onCancel}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Recipe Name</label>
            <input
                name="name"
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><Image size={14} /> Image URL</label>
            <input
                name="image_url"
                placeholder="https://..."
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
             ...
             <div>
                ...
                <input name="protein_g" type="number" placeholder="0" value={formData.protein_g} onChange={handleChange} className="w-full px-4 py-2 rounded-lg bg-gray-50 border border-gray-200" />
             </div>
             <div>
                <label className="block text-xs font-bold text-orange-600 uppercase mb-1">Carbs (g)</label>
                <input name="carbs_g" type="number" placeholder="0" value={formData.carbs_g} onChange={handleChange} className="w-full px-4 py-2 rounded-lg bg-gray-50 border border-gray-200" />
             </div>
             <div>
                <label className="block text-xs font-bold text-yellow-600 uppercase mb-1">Fat (g)</label>
                <input name="fat_g" type="number" placeholder="0" value={formData.fat_g} onChange={handleChange} className="w-full px-4 py-2 rounded-lg bg-gray-50 border border-gray-200" />
             </div>
          </div>

          <div>
             <label className="block text-xs font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><List size={14} /> Instructions</label>
             <textarea
                name="instructions"
                placeholder="Step 1..."
